...
public class OAuthUtils {
    ...
    public static Resource getProtectedResource(OAuth2Client client, Token token, String path) {
        String resourceURL = client.getConfig().getSite() + path;
        ...
        return new Resource(responseString, code);
    }

    public static Resource getProtectedResourcePost(OAuth2Client client, Token token, String path, List<BasicNameValuePair> parametersBody) {
        String resourceURL = client.getConfig().getSite() + path;
        HttpPost post = new HttpPost(resourceURL);
        post.addHeader(OAuthConstants.AUTHORIZATION, getAuthorizationHeaderForAccessToken(token.getAccessToken()));
        DefaultHttpClient httpClient = new DefaultHttpClient();
        HttpResponse response = null;
        String responseString = "";
        int code = -1;
